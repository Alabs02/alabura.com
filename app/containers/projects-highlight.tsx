    ...
    >
      ...
      <div className="flex flex-col justify-between border-none items-start transition-all duration-300 relative will-change-transform min-h-[calc(60%-32px)] group-hover/bento:min-h-[60%] overflow-hidden">
        ...
        <Link
          ...
          aria-label="See Full Overview"
          className="block w-full transition-all duration-300 will-change-transform translate-y-12 group-hover/bento:translate-y-0 delay-75"
          passHref
        >
...
const GridHeroPattern = memo(() => {
  ...
  return (
    <>
      ...
      >
        {columns.map((col) => (
          <div key={`col-${col}`} className="grid grid-rows-8 -mt-12">
            {rows.map((row) => (
              <motion.div
                key={`row-${col}-${row}`}
                className={cn(
